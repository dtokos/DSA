...
static const int HT_PRIME_3 = 179;
...
static ht_hash_table* ht_new_sized(const int size_index) {
	...
	ht->count = 0;
	ht->items = (ht_item **)xcalloc((size_t)ht->size, sizeof(ht_item*));
	return ht;
}
...
static int ht_generic_hash(const char* s, const int a, const int m) {
	long hash = 0;
	const int len_s = (const int)strlen(s);
...
static int ht_hash(const char* s, const int num_buckets, const int attempt) {
	...
	return (hash_b + 1) == num_buckets ?
		(hash_a + (attempt * (ht_generic_hash(s, HT_PRIME_3, num_buckets) + 1))) % num_buckets :
